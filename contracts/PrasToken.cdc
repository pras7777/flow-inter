...
pub contract PrasToken: FungibleToken {
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, CollectionPublic {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @PrasToken.Vault
        ...
        // Destroy the vault
        destroy() {
            PrasToken.totalSupply = PrasToken.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            PrasToken.totalSupply = PrasToken.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&PrasToken.Minter>(/public/Minter, target: /storage/MinterStorage)
